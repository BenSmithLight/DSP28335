...
interrupt void ISRMcbspSend(void) // 发送中断程序
{
	...
	temp1 = McbspaRegs.DRR1.all;				   // 将数据读入temp1
	McbspaRegs.DXR1.all = temp1;				   // 将temp1中的数据写入发送寄存器
	unsigned char data[2] = {(temp1 >> 8), temp1}; // 语音数据的高低八位分别传入数组中
	unsigned char crc = crc8(data, sizeof(data));  // 计算CRC校验码
	sdata[0] = BAOTOU;							   // 帧头
	sdata[1] = data[0];							   // 高八位数据
	sdata[2] = data[1];							   // 低八位数据
	sdata[3] = crc;								   // CRC校验值
	sdata[4] = BAOWEI;							   // 帧尾
	Uint16 cnt;									   // 定义cnt
	for (cnt = 0; cnt < 5; cnt++)
	{
		scib_xmit(sdata[cnt]); // 发送数据
	}
